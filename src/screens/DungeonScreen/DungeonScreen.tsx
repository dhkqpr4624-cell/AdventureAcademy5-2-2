...
import { DungeonReturnPrompt } from "../../components/MemoryFragmentEvent";
...
import { DUNGEON3_ENTRY_STORY, DUNGEON3_FINAL_STORY } from "../../data/stories/dungeon3Chapter2Stories";
    ...
    rooms: map.rooms.map((room) => {
      if (room.type === "combat" && room.combatConfig) {
        const monsterId = floorId === "floor-1"
          ...
          : floorId === "floor-2" ? "chapter2-angry-cheonin"
          : floorId === "floor-3"
            ? "chapter2-broken-angbuilgu"
            : floorId === "floor-4"
        ...
        return {
          ...
          eliteConfig: {
            ...
            monsterId: floorId === "floor-1"
              ...
              : floorId === "floor-2" ? "chapter2-angry-yangin"
              : floorId === "floor-3"
                ? "chapter2-corrupted-armillary-sphere"
                : floorId === "floor-4"
  ...
  const floorMap = floorId === "floor-2" || floorId === "floor-6" || floorId === "floor-7" || floorId === "floor-9" ? {
    ...
    rooms: baseMap.rooms.map((room, _index, rooms) => {
      const selectedIds = floorId === "floor-2"
        ? selectDungeon2StoryRoomIds({ ...baseMap, rooms })
        : floorId === "floor-9"
...
export function DungeonScreen({
  ...
  const [floor2EntryStoryVisible, setFloor2EntryStoryVisible] = useState(false);
  const [floor2ClueStoryIndex, setFloor2ClueStoryIndex] = useState<number | null>(null);
  const [floor2FinalStoryVisible, setFloor2FinalStoryVisible] = useState(false);
  const [floor3EntryStoryVisible, setFloor3EntryStoryVisible] = useState(false);
  ...
  const playEquippedWeaponVfx = async () => {
    const weaponId = inventoryState.equippedItemIds.weaponSkin;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      if (questStoryEnabled && floorId === "floor-2" && floorQuestStarted && currentRoomId === dungeonMap.startRoomId && !roomProgressRef.current[dungeonMap.startRoomId]?.eventCompleted) {
        setDungeonMode("roomEvent");
        setFloor2EntryStoryVisible(true);
      }
      if (questStoryEnabled && floorId === "floor-3" && floorQuestStarted && currentRoomId === dungeonMap.startRoomId && !roomProgressRef.current[dungeonMap.startRoomId]?.eventCompleted) {
        setDungeonMode("roomEvent");
        setFloor3EntryStoryVisible(true);
      }
      if (questStoryEnabled && floorId === "floor-4" && floorQuestStarted && currentRoomId === dungeonMap.startRoomId && !roomProgressRef.current[dungeonMap.startRoomId]?.eventCompleted) {
        setDungeonMode("roomEvent");
      ...
      if (questStoryEnabled && floorId === "floor-7" && floorQuestStarted && currentRoomId === dungeonMap.startRoomId && !roomProgressRef.current[dungeonMap.startRoomId]?.eventCompleted) {
        setDungeonMode("roomEvent");
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      dungeon4Environment?.update(clock.elapsedTime);
      weapon.update(delta);
  ...
  const playSword = (attackType: WeaponAttackType): Promise<void> => {
    ...
    return new Promise((resolve) => {
      ...
      const recordHit = () => {
        recordResult();
        // Chapter2 weapon hit feedback is committed after the attack VFX.
      };
  ...
  const playPlayerAttack = async (isCorrect: boolean) => {
    ...
    if (isCorrect) {
      ...
      const hitAnimation = visualsRef.current?.monster.play(
  ...
  const advanceCombatMessage = async () => {
    ...
    try {
      ...
      if (phase === "awaitStagger") {
        ...
        const hitAnimation = visualsRef.current?.monster.play("hit");
        playRandomizedOneShot(HIT_SFX_URL);
        await hitAnimation;
        await playVictory();
        return;
  ...
  const handleRoomEntered = (roomId: string) => {
    ...
    try {
      ...
      if (questStoryEnabled && floorId === "floor-2") {
        const clueIndex = selectDungeon2StoryRoomIds(dungeonMap).indexOf(roomId);
        if (clueIndex >= 0 && !roomProgressRef.current[roomId]?.eventCompleted) {
          setFloor2ClueStoryIndex(clueIndex);
          return;
        }
        if (floorQuestStarted && isFinalRoom(dungeonMap, roomId) && !firstObjectiveEventSeen) {
          setFloor2FinalStoryVisible(true);
          return;
        }
      }
      if (questStoryEnabled && floorId === "floor-3" && floorQuestStarted && isFinalRoom(dungeonMap, roomId) && !firstObjectiveEventSeen) {
        setFloor3FinalStoryVisible(true);
        return;
      }
      if (questStoryEnabled && floorId === "floor-4" && floorQuestStarted && isFinalRoom(dungeonMap, roomId) && !firstObjectiveEventSeen) {
  ...
  const restartTestDungeon = (restoreHp = true) => {
    ...
    if (floorId === "floor-2" && nextProgress[dungeonMap.startRoomId]) {
      nextProgress[dungeonMap.startRoomId] = { roomId: dungeonMap.startRoomId, eventCompleted: false };
    }
    roomProgressRef.current = nextProgress;
    setRoomProgress(nextProgress);
    setActiveCombatRoomId(null);
    ...
    if (floorId === "floor-2" && floorQuestStarted) {
      setDungeonMode("roomEvent");
      setFloor2EntryStoryVisible(true);
    }
    initializeCombatEncounterState();
  ...
  return (
    ...
    >
      ...
      {floor2EntryStoryVisible && floorId === "floor-2" && <div className="dungeon-story-overlay">
        <StoryPlayer sequence={DUNGEON2_ENTRY_STORY} playerName={playerState.name || DEFAULT_PLAYER_NAME} playerStatus={playerState} presentationMode="baseCampOverlay" onNavigate={onNavigate} onComplete={() => {
          const next = completeRoomEvent(roomProgressRef.current, dungeonMap.startRoomId); roomProgressRef.current = next; setRoomProgress(next); setFloor2EntryStoryVisible(false); setDungeonMode("exploration");
        }} />
      </div>}
      {floor2ClueStoryIndex !== null && floorId === "floor-2" && <div className="dungeon-story-overlay">
        <StoryPlayer sequence={DUNGEON2_CLUE_STORIES[floor2ClueStoryIndex]!} playerName={playerState.name || DEFAULT_PLAYER_NAME} playerStatus={playerState} presentationMode="baseCampOverlay" onNavigate={onNavigate} onComplete={() => {
          const roomId = selectDungeon2StoryRoomIds(dungeonMap)[floor2ClueStoryIndex];
          if (roomId) { const next = completeRoomEvent(roomProgressRef.current, roomId); roomProgressRef.current = next; setRoomProgress(next); }
          setFloor2ClueStoryIndex(null); setDungeonMode("exploration");
        }} />
      </div>}
      {floor2FinalStoryVisible && floorId === "floor-2" && <div className="dungeon-story-overlay">
        <StoryPlayer sequence={DUNGEON2_FINAL_STORY} playerName={playerState.name || DEFAULT_PLAYER_NAME} playerStatus={playerState} presentationMode="baseCampOverlay" onNavigate={onNavigate} onComplete={() => {
          playerHpRef.current = maxHp; setPlayerHp(maxHp); setFloor2FinalStoryVisible(false); onStoryEventSeen("floor-2:chapter2-final"); onObjectiveAcquired(runCorrectCountRef.current); onFloorCleared(); onNavigate("baseCamp");
        }} />
      </div>}
      {floor3EntryStoryVisible && floorId === "floor-3" && <div className="dungeon-story-overlay">
        <StoryPlayer sequence={DUNGEON3_ENTRY_STORY} playerName={playerState.name || DEFAULT_PLAYER_NAME} playerStatus={playerState} presentationMode="baseCampOverlay" onNavigate={onNavigate} onComplete={() => {
          const next = completeRoomEvent(roomProgressRef.current, dungeonMap.startRoomId); roomProgressRef.current = next; setRoomProgress(next); setFloor3EntryStoryVisible(false); setDungeonMode("exploration");
        }} />
      </div>}
      {floor3FinalStoryVisible && floorId === "floor-3" && <div className="dungeon-story-overlay">
        <StoryPlayer sequence={DUNGEON3_FINAL_STORY} playerName={playerState.name || DEFAULT_PLAYER_NAME} playerStatus={playerState} presentationMode="baseCampOverlay" onNavigate={onNavigate} onComplete={() => {
          playerHpRef.current = maxHp; setPlayerHp(maxHp); setFloor3FinalStoryVisible(false); onStoryEventSeen("floor-3:chapter2-final"); onObjectiveAcquired(runCorrectCountRef.current); onFloorCleared(); onNavigate("baseCamp");
        }} />
      </div>}
      {floor4EntryStoryVisible && floorId === "floor-4" && <div className="dungeon-story-overlay">
        <StoryPlayer sequence={DUNGEON4_ENTRY_STORY} playerName={playerState.name || DEFAULT_PLAYER_NAME} playerStatus={playerState} presentationMode="baseCampOverlay" onNavigate={onNavigate} onComplete={() => {
          const next = completeRoomEvent(roomProgressRef.current, dungeonMap.startRoomId); roomProgressRef.current = next; setRoomProgress(next); setFloor4EntryStoryVisible(false); setDungeonMode("exploration");
        }} />
      </div>}
      {floor4FinalStoryVisible && floorId === "floor-4" && <div className="dungeon-story-overlay">
        <StoryPlayer sequence={DUNGEON4_FINAL_STORY} playerName={playerState.name || DEFAULT_PLAYER_NAME} playerStatus={playerState} presentationMode="baseCampOverlay" onNavigate={onNavigate} onComplete={() => {
      ...
      {floor7EntryStoryVisible && floorId === "floor-7" && <div className="dungeon-story-overlay">
        <StoryPlayer sequence={DUNGEON7_ENTRY_STORY} playerName={playerState.name || DEFAULT_PLAYER_NAME} playerStatus={playerState}
          presentationMode="baseCampOverlay" onNavigate={onNavigate} onComplete={() => {
            const next = completeRoomEvent(roomProgressRef.current, dungeonMap.startRoomId);
      ...
      {objectiveEvent === "first" && floorId === "floor-7" && <div className="dungeon-story-overlay">
        <StoryPlayer sequence={DUNGEON7_FINAL_STORY} playerName={playerState.name || DEFAULT_PLAYER_NAME} playerStatus={playerState}
          presentationMode="baseCampOverlay" onNavigate={onNavigate} onComplete={() => {
            setObjectiveEvent(null); setFloor7RescueStoryVisible(true);
          }} />
      </div>}
      {floor7RescueStoryVisible && floorId === "floor-7" && <Dungeon7RescueStory playerName={playerState.name || DEFAULT_PLAYER_NAME} onComplete={() => {
        playerHpRef.current = maxHp; setPlayerHp(maxHp); onStoryEventSeen("floor-7:deneb-rescued");
        onObjectiveAcquired(runCorrectCountRef.current); setFloor7RescueStoryVisible(false); onFloorCleared(); onNavigate("baseCamp");
      }} />}
